                      ...
                      <div className="space-y-2">
                        {selectedTender.documents.map((doc) => (
                          <a
                            ...
                            className="flex items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white p-3 sm:p-4 hover:bg-gray-100 active:bg-gray-200 transition min-h-[52px]"
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <span className="text-blue-500 text-lg flex-shrink-0">📄</span>
                              <div className="min-w-0">
                                <div className="font-medium text-gray-900 text-sm break-all">{doc.file_name}</div>
                                <div className="text-xs text-gray-500">Uploaded: {new Date(doc.uploaded_at).toLocaleDateString()}</div>
                              </div>
                            </div>
                            <span className="text-accent text-xs font-medium flex-shrink-0">Open ↗</span>
                          </a>
                        ))}
                      </div>
                    </div>
